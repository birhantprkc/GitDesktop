...
- **GitLab too** — via the **GitLab CLI (`glab`)**, browse and clone your GitLab
  ...
  **or merge request**, edit an issue's or MR's **labels** and an issue's **assignees**,
  **approve / unapprove** and **merge** (merge/squash) an MR, plus **create issues and
  merge requests** (push-and-open, drafts included — publishing local issues/PRs to
  GitLab works too), right from the app. Other GitLab surfaces (full MR reviews,
  editing an issue's title/body, …) read "coming soon" for now; GitHub is unchanged.
