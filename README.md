...
- **GitLab too (read)** — via the **GitLab CLI (`glab`)**, browse and clone your
  GitLab projects and read **merge requests**, **issues**, **CI pipelines**, and
  **releases** in the same panels: lists, plus read-only detail views (MR
  comments/commits/diff; issue comments with a labels/assignees/milestone rail;
