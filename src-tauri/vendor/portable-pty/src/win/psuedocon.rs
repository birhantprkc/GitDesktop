...
impl PsuedoCon {
    ...
    pub fn spawn_command(&self, cmd: CommandBuilder) -> anyhow::Result<WinChild> {
        ...
        // GitDesktop vendored patch: pass NULL (not INVALID_HANDLE_VALUE) for the
        // child's std handles. With STARTF_USESTDHANDLES, Windows substitutes the
        // pseudoconsole's CONIN$/CONOUT$ only for std handles that are NULL — an
        // INVALID_HANDLE_VALUE is passed through verbatim and never replaced (see
        // rprichard's win32-console-docs; alacritty + microsoft/terminal DHowett use
        // NULL). With INVALID_HANDLE_VALUE the spawned child (e.g. cmd.exe/docker.exe)
        // gets no valid console handles, allocates its OWN console (a stray window)
        // instead of attaching to the pseudoconsole, and the pty's output pipe is
        // empty and EOFs immediately. NULL still blocks inheriting the parent's
        // redirected handles (the original intent below). Upstream still uses
        // INVALID_HANDLE_VALUE; drop this vendor if/when wezterm fixes it.
